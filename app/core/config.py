...
_CHAVES_POR_SECAO: Dict[str, List[str]] = {
    ...
    "avancado": ["logs", "json_audit", "debug", "web"],
    "urls": ["urls"],
}
...
def validar_settings(settings: Dict[str, Any]) -> List[str]:
    ...
    porta = web.get("porta", 8765)
    if not (isinstance(porta, int) and not isinstance(porta, bool) and 1024 <= porta <= 65535):
        problemas.append("Porta da Interface Web deve ser um número entre 1024 e 65535.")
...
def salvar_segredos_notificacao(telegram_token: str, telegram_chat_id: str, ntfy_topic: str, ntfy_servidor: str = "") -> None:
    """
