...
from app.core.config import aplicar_segredos_notificacao_salvos, carregar_disciplinas, carregar_settings, salvar_settings
...
class SniperApp(tk.Tk):
    ...
    def recarregar_telas(self):
        """Atualiza os campos das telas a partir de self.settings/self.disciplinas/
        self.sessao. As telas são construídas uma única vez; sem isto, depois do
        assistente de primeira execução, de "Restaurar padrões" ou de "Importar",
        elas continuavam mostrando os valores antigos (bug real: na aba
        Disciplinas a lista ficava desatualizada e "Remover"/"Editar" agiam sobre
        outra disciplina; na aba Avançado, "Salvar" regravava os valores antigos
        por cima dos restaurados)."""
        for tela in self._telas.values():
            recarregar = getattr(tela, "recarregar", None)
            if recarregar:
                try:
                    recarregar()
                except Exception:
                    pass  # atualizar a tela nunca pode derrubar a GUI
